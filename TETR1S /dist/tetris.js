...
function addControlEvent(elementId, eventName, action) {
    const element = document.getElementById(elementId);

    // Pastikan elemen ada sebelum menambahkan event listener
    if (element) {
        // Tambahkan event listener untuk klik mouse dan touchstart
        element.addEventListener('click', action);
        element.addEventListener('touchstart', (event) => {
            event.preventDefault(); // Mencegah efek sentuhan ganda
            action();
        });
    } else {
...
// Hubungkan tombol dengan aksi yang sesuai
addControlEvent('left', 'ArrowLeft', () => tetris.moveLeft());
addControlEvent('right', 'ArrowRight', () => tetris.moveRight());
addControlEvent('down', 'ArrowDown', () => tetris.moveDown());
addControlEvent('up', 'ArrowUp', () => tetris.rotatePiece()); // Tombol 'up' untuk memutar blok

// Kelas Tetris
class Tetris {
    constructor() {
        this.currentPiece = null; // Inisialisasi balok saat ini
        this.grid = []; // Inisialisasi grid permainan
        // Tambahkan logika untuk menginisialisasi grid dan memulai permainan
    }
    ...
    rotatePiece() {
        if (!this.currentPiece) return; // Pastikan balok aktif tersedia

        // Simpan state awal balok
        const originalState = { 
        ...
        // Hitung matriks hasil rotasi
        const rotatedMatrix = this.rotateMatrix(this.currentPiece.shape);

        // Cek apakah rotasi valid
        if (this.canRotate(rotatedMatrix)) {
            this.currentPiece.shape = rotatedMatrix; // Tetapkan rotasi baru
            this.updateGame(); // Perbarui tampilan game
            console.log('Piece rotated successfully!');
        } else {
            // Kembalikan ke state awal jika rotasi tidak valid
            this.currentPiece = originalState;
            console.log('Rotation failed, reverting to original state');
        }
    }
        ...
        for (let row = 0; row < rotatedMatrix.length; row++) {
            for (let col = 0; col < rotatedMatrix[row].length; col++) {
                if (rotatedMatrix[row][col]) {
                    ...
                    // Cek apakah keluar dari batas grid atau menabrak balok lain
                    if (
                        newX < 0 || 
    ...
    canMove(deltaX, deltaY) {
        // Cek apakah posisi baru valid di grid
        // Implementasi bergantung pada sistem permainan Anda
    }

    updateGame() {
        // Render ulang permainan
    }
    ...
    spawnNewPiece() {
        // Memunculkan blok baru
    }

    loadState(state) {
        // Memuat state sebelumnya untuk undo
    }
}
...
// Deklarasikan tetris sebagai variabel global
let gameInstance;
...
var calcIntervalByLevel = function(level){
	return consts.DEFAULT_INTERVAL  - (level-1)*120;
};
...
function Tetris(id){
    ...
    gameInstance = this;
	this.previousStates = []; // Tambahkan array untuk menyimpan state sebelumnya
}
...
Tetris.prototype = {
	...
	init:function(options){
		...
		// Tambahkan binding untuk kontrol tombol setelah inisialisasi
        this._initControls();
	...
	// Tambahkan method untuk inisialisasi kontrol
    _initControls: function() {
        const addControlEvents = (buttonId, action) => {
        ...
        // Bind tombol dengan method yang sesuai
        addControlEvents('left', () => this.moveLeft());
        addControlEvents('right', () => this.moveRight());
        addControlEvents('down', () => this.moveDown());
        addControlEvents('up', () => this.rotatePiece());
    },
    ...
    // Implementasi fungsi undo yang benar
    _saveState: function() {
        const state = {
            ...
            shape: this.shape ? {
                x: this.shape.x,
                y: this.shape.y,
                state: this.shape.state,
                flag: this.shape.flag,
                color: this.shape.color
            } : null,
            score: this.score
        };
        this.previousStates.push(state);
        // Batasi jumlah state yang disimpan
    ...
    undoLastMove: function() {
        if (!this.isGameOver && this.previousStates.length > 0) {
            const previousState = this.previousStates.pop();
            
            // Restore matrix
            this.matrix = previousState.matrix;
            
            // Restore shape
            if (previousState.shape && this.shape) {
                this.shape.x = previousState.shape.x;
                this.shape.y = previousState.shape.y;
                this.shape.state = previousState.shape.state;
                this.shape.color = previousState.shape.color;
            }
    ...
    // Modifikasi method yang mempengaruhi state game untuk menyimpan state
    moveLeft: function() {
        if (!this.isGameOver && this.shape) {
            this._saveState();
